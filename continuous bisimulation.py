...
# what does L mean???
    ...
    while np.sum(IJ) > 0:
        ...
        diff_flag = True
        if (vol1 > min_cell_volume) and (risect > rd):
            for sk in sol:
                if sk == sj:
                    continue
                #diff = sk.diff(S0)
                ...
                diffk = sk.diff(isect)
                volk = diffk.volume
                rdiffk, xdk = pc.cheby_ball(diffk)
                if not((volk > min_cell_volume) and (rdiffk > rd)):
                    diff_flag = False
                    break
        else:
            diff_flag = False
        if diff_flag == True:
            ...
            num_new = len(isect_list)

            # add each piece, as a new state
            # step 11?
            for region in isect_list:
                sol.append(region)
